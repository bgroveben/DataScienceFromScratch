...
def random_x_given_y(y):
    if y <= 7:
        # if the total is 7 or less, the first die is equally
        # likely to be 1, 2, ..., (total - 1)
        return random.randrange(1, y)
    else:
        # if the total is 7 or more, the first die is equally
        # likely to be (total - 6), (total - 5), ..., 6
        return random.randrange(y - 6, 7)


def gibbs_sample(num_iters=100):
    x, y = 1, 2  # it doesn't really matter which numbers are used
...
def sample_from(weights):
    total = sum(weights)
    rnd = total * random.random()     # uniform betwen 0 and total
    for i, w in enumerate(weights):
        rnd -= w                      # return the smalles i such that
...
def p_topic_given_document(topic, d, alpha=0.1):
    """ the fraction of words in document _d_ that are assigned to _topic_ (plus smoothing) """
    return ((document_topic_counts[d][topic] + alpha) /
             (document_lengths[d] + K * alpha))


def p_word_given_topic(word, topic, beta=0.1):
    """ the fraction of words assigned to _topic_ that equal _word_ (plus smoothing) """
