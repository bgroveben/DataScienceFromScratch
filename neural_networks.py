...
def feed_forward(neural_network, input_vector):
    """ takes a neural network (represented as a list of lists of lists of weights) """
    """ and returns the output from forward-propagating the input """
    outputs = []
    for layer in neural_network:
        input_with_bias = input_vector + [1]              # add a bias input,
        output = [neuron_output(neuron, input_with_bias)  # calculate the output
                  for neuron in layer]                    # for this layer,
        outputs.append(output)                            # and store it
    ...
    for i in range(5):                # row
        for j in range(5):            # column
            if weights[5*i + j] < 0:  # row i, column j = weights[5*i + j]
                # add black and white hatches for visibility
...
if __name__=="__main__":
    ...
    def make_digit(raw_digit):
        result = [1 if c == '1' else 0 for row in raw_digit.split("\n") for c in row.strip()]
        print result
        return result
    ...
    # 10,000 iterations should be enough to converge
