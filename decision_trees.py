...
def partition_by(inputs, attribute):
    ...
    """ each input is a pair (attibute_dict, label) """
...
def classify(tree, input):
    ...
    # if this is a leaf node, return it values
...
def build_tree_id3(inputs, split_candidates=None):
    ...
    if num_trues == 0:                   # if only Falses remain,
        return False                     # return a "False" leaf
    if num_falses == 0:                  # if only Trues remain,
        return True                      # return a "True" leaf
    if not split_candidates:             # if no split candidates remain,
