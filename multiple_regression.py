...
def bootstrap_statistic(data, stats_fn, num_samples):
    """ evaluates stats_fn on num_samples bootstrap samples form data """
...
# sometimes it's called "lambda", but that already has a different meaning in Python
def ridge_penalty(beta, alpha):
    return alpha * dot(beta[1:], beta[1:])


def squared_error_ridge(x_i, y_i, beta, alpha):
    """ estimate error plus ridge penaly on beta """
...
def estimate_beta_ridge(x, y, alpha):
    """ use gradient descent to fit a ridge regression with penaly alpha """
    beta_initial = [random.random() for x_i in x[0]]
